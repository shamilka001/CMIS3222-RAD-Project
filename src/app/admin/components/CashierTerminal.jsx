...
//     } else {
//       const seatPrice =
//         seat.type === "VIP Recliner"
//           ? selectedShowtime.priceVip
//           : selectedShowtime.priceNormal;
...
//                   }`}
//                 >
//                   <div className="text-2xl p-2 bg-input rounded-xl border border-border/80">
//                     {movie.poster}
//                   </div>
...
//                         Normal: ${showtime.priceNormal.toFixed(2)}
//                       </div>
//                       <div className="text-amber-400">
//                         VIP: ${showtime.priceVip.toFixed(2)}
//                       </div>
//                     </div>
//                   </button>
//                 ))}
...
//               {selectedShowtime.time} ({selectedShowtime.screen})
//             </div>
//             <div>
...
export default function CashierTerminal() {
  ...
  useEffect(() => {
    try {
      ...
      if (storedUser) {
        ...
        // Fallback safely if id isn't explicitly found inside the decoded token root
  ...
  useEffect(() => {
    ...
    async function fetchShowtimes() {
      try {
        ...
        const json = await res.json();
        if (json.data) {
          // Rule: Filter out past dates so admin cannot accidentally click/book past shows
          const now = new Date();
  ...
  useEffect(() => {
    ...
    async function fetchSeatingMatrix() {
      try {
        setLoading(true);
        // 1. Fetch physical layout of the Screen
        const layoutRes = await fetch(
          `${API_BASE}/seat/screen/${selectedShowtime.screen_id}`,
        );
        const layoutJson = await layoutRes.json();

        // 2. Fetch already occupied seats for this particular timeframe
        ...
        if (layoutJson.data) {
          // Construct Row Maps dynamically from tabular structure
  ...
  // Bind dynamic halftone engine to background cover selection
  ...
  const toggleSeatSelection = (seat) => {
    ...
    if (selectedSeats.find((s) => s.id === seat.id)) {
      ...
      const seatPrice = seat.type === "VIP" ? 25.0 : 15.0;
      setSelectedSeats([...selectedSeats, { ...seat, price: seatPrice }]);
    }
  };
  ...
  const executeOrderCreation = async () => {
    try {
      ...
      // Dynamically uses currentUserId sourced directly from the active localStorage session!
      const payload = {
      ...
      const response = await fetch(`${API_BASE}/booking/save`, {
        ...
        headers: {
          ...
          ...(token && { Authorization: `Bearer ${token}` }), // Passes auth state if required by back-end rules
        },
