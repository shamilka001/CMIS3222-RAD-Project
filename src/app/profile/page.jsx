...
//           const payload = JSON.parse(atob(token.split(".")[1]));
//           userId = payload.id;
//           userRole = payload.role; // Extract role from JWT token
...
//           // 🛑 ADMIN GUARD: Redirect admins away from customer profile
//           if (role?.toUpperCase() === "STAFF") {
//             router.replace("/admin"); // Change to your actual admin route (e.g., /admin/dashboard)
//             return;
//           }
    ...
    const fetchData = async () => {
      ...
      try {
        ...
        if (userRes.ok && responseData?.data) {
          ...
          // 🛑 ADMIN/STAFF GUARD: Redirect admins and staff away from customer profile
          if (role && staffRoles.includes(String(role).toUpperCase())) {
