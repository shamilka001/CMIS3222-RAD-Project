 ...
 // Dynamic PDF Generator function - Fixed overlapping layout
  async function generateAndDownloadTicketPDF(bookingId, seats, totalCost) {
    try {
      ...
      // Ticket Styling and Colors
      const primaryColor = [6, 182, 212]; // cyan-500
      const darkColor = [24, 24, 27];     // zinc-900

      // Outer Ticket Border / Background Accent
      doc.setDrawColor(...primaryColor);
      doc.setLineWidth(1);
      doc.rect(4, 4, 105 - 8, 148 - 8); // ticket borders

      // Header Banner
      doc.setFillColor(...darkColor);
      doc.rect(5, 5, 105 - 10, 25, "F");
      ...
      // Metadata Info Block (Stacked Layout to prevent long ID overlaps)
      ...
      // Seats Info Block (Centered Highlighted Box)
      doc.setFont("Helvetica", "bold");
      doc.setFontSize(9);
      doc.text("CLAIMED SEATS:", 10, 88);
      
      doc.setFillColor(244, 244, 245);
      doc.rect(10, 92, 85, 12, "F"); // Spans full width of content area
      doc.setTextColor(...primaryColor);
      ...
      doc.setFont("Helvetica", "normal");
      doc.setFontSize(9);
      doc.text("CREDIT CARD (MOCK)", 45, 112); // Short string, fine side-by-side
  ...
  async function handleMockPaymentSubmit(e) {
    ...
    const ticketCost = selectedSeats.length * 1000;
    const paymentPayload = {
      bookingId: currentBookingId,
      amount: ticketCost,
      paymentMethod: "CARD",
